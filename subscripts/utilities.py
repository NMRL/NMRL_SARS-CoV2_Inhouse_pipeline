...
class Housekeeper:
    ...
    aa_encodings = [ #used by convert_aa_encodings
        ('Val','V'),
        ('Ile','I'),
        ('Leu','L'),
        ('Glu','E'),
...
### Edit for this pipeline
    # @staticmethod 
    # def name_ardetype_logs(path_to_log_dir:str=f"{os.path.dirname(Path(__file__).parents[0].absolute())}/ardetype_job_logs"):
    #     '''Given path to the ardetype_job_logs folder, adds sample id to the name of each log file (skips the file if sample id is not found in it).'''
    #     os.chdir(path_to_log_dir)
    #     log_list = [f for f in os.listdir("./") if "_ardetype" not in f and "_default" not in f]
    #     for log in log_list:
    #         try:
